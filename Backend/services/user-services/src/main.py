...
from src.database.connection import get_users_db, get_public_db, get_users_db
...
def add_announcment(request: AddAnnouncementRequest, db: Session = Depends(get_public_db), db_user: Session = Depends(get_users_db)):
    ...
    try:
        ...
        if not author_role or author_role not in ["admin", "council", "club"]: 
            return JSONResponse(content= {'type': "error", "details": "Unauthorized User, critical!"})
        ...
        db.commit()
        return JSONResponse(content= {'type':"ok", 'details': "Announcement added"})
    except Exception as e:
...
def get_own_announcements(request: OwnAnnouncementRequest,
    ...
    try:
        ...
        return JSONResponse(content= {'type': "error", 'details': "Failed to retrieve announcements"})
...
def add_announcment(request: UpdateAnnouncementRequest, db: Session = Depends(get_public_db)):
    ...
    try:
        ...
        if not existing_announcement:
            return JSONResponse(content= {'type':"error", 'details': "Announcement not found"})
        ...
        return JSONResponse(content= {'type':"ok", 'details': "Announcement updated",'id': existing_announcement.id})
    except Exception as e:
        print("ERROR in add announcement:", e)
        return JSONResponse(content={'type': "error", 'details':"An error occurred"})
