...
engine_public = create_engine(PUBLIC_DATABASE_URL, pool_size=10, max_overflow=5, pool_timeout=30, pool_recycle=1800, echo=True)
SessionPublic = sessionmaker(autocommit=False, autoflush=False, bind=engine_public)

engine_project = create_engine(PROJECTS_DATABASE_URL, pool_size=5, max_overflow=2, pool_timeout=20, pool_recycle=1800, echo=True)
